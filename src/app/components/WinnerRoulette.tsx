...
export function WinnerRoulette({ 
  ...
  const spinRoulette = () => {
    ...
    const winningIndex = Math.floor(((360 - (newRotation % 360)) % 360) / sliceAngle)
    ...
    <div className="flex flex-col items-center justify-center min-h-[85vh] w-full max-w-md mx-auto">
      <div className="w-full bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col relative z-10">
        ...
        <div className="p-4 sm:p-5 flex justify-between items-center bg-white gap-2">
          ...
          {!isSpectator ? (
            <div className="flex gap-2">
              <Button onClick={onResetGame} disabled={isSpinning} className="bg-[#FBBF24] hover:bg-[#F59E0B] text-black font-bold rounded-xl border-2 border-[#F59E0B] px-3 sm:px-4 h-10 flex items-center gap-1.5 text-xs sm:text-sm"><RotateCcw className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" /><span className="hidden sm:inline">Reintegrar</span></Button>
              <Button onClick={onBack} disabled={isSpinning} className="bg-[#FB7185] hover:bg-[#F43F5E] text-black font-bold rounded-xl border-2 border-[#F43F5E] px-3 sm:px-4 h-10 flex items-center gap-1.5 text-xs sm:text-sm"><LogOut className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" /><span className="hidden sm:inline">Salir</span></Button>
            </div>
          ) : (
        ...
        <div className="p-6 flex flex-col items-center relative flex-1">
          <div className="relative z-20 mb-[-10px]"><div className="w-0 h-0 border-l-[16px] border-r-[16px] border-t-[24px] border-l-transparent border-r-transparent border-t-black drop-shadow-md"></div></div>

          <div className="relative w-[300px] h-[300px] sm:w-[340px] sm:h-[340px] mb-8">
            <canvas ref={canvasRef} width={800} height={800} className="w-full h-full rounded-full drop-shadow-xl" style={{ transform: `rotate(${rotation}deg)`, transition: isSpinning ? 'transform 5s cubic-bezier(0.2, 0.8, 0.2, 1)' : 'none' }} />
          </div>
