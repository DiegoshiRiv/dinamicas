  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-4 sm:p-6 relative flex flex-col">
      <div className="max-w-6xl mx-auto flex-1 w-full">
        ...
        {/* Botón de Admin ajustado para no chocar en celulares */}
        <div className="flex justify-end w-full mb-4">
          {!isAdmin ? (
            <button onClick={() => setShowLogin(true)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition shadow-md text-sm font-medium">
              <LogIn className="w-4 h-4" /> Admin
            </button>
          ) : (
            <button onClick={handleLogout} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-600 text-white hover:bg-red-700 transition shadow-md text-sm font-medium">
        ...
        <header className="text-center mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-800 tracking-tight leading-tight px-2">
            Dinámicas Pokémon GO GDL
          </h1>
        </header>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          {/* Pestañas responsivas con flex-wrap */}
          <TabsList className={`flex flex-wrap justify-center w-full mx-auto mb-8 h-auto gap-2 bg-transparent sm:bg-slate-100 p-1 sm:rounded-xl ${isAdmin ? 'max-w-4xl' : 'max-w-lg'}`}>
            <TabsTrigger value="register" className="flex-1 min-w-[140px] flex items-center justify-center gap-2 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg">
              <Users className="h-4 w-4" /> Registro
            </TabsTrigger>
            
            <TabsTrigger value="sponsors" className="flex-1 min-w-[140px] flex items-center justify-center gap-2 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg">
              <Heart className="h-4 w-4 text-pink-500" /> Patrocinadores
            </TabsTrigger>
            
            {isAdmin && (
              <>
                <TabsTrigger value="admin" className="flex-1 min-w-[140px] flex items-center justify-center gap-2 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg">
                  <Trophy className="h-4 w-4" /> Administración
                </TabsTrigger>
                <TabsTrigger value="qr" className="flex-1 min-w-[140px] flex items-center justify-center gap-2 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg">
          ...
          <TabsContent value="sponsors">
             <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 sm:p-10 border border-gray-100">
               <div className="text-center mb-8 sm:mb-12">
                 ...
                 <p className="text-sm sm:text-base text-gray-500 max-w-xl mx-auto leading-relaxed px-2">
                   Gracias al apoyo incondicional de estas cuentas, nuestras dinámicas y premios son posibles. ¡Toca su foto para ir a seguirlos en Instagram!
                 </p>
               </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={onLoginSubmit}>
              ...
              <div className="relative mb-4">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-2" tabIndex={-1}>
